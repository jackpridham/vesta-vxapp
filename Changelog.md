Vortex vesta-vxapp Compose Orchestrator [2026-07-25]
==================================================
* Implemented a secure single-host Docker Compose orchestrator for Vesta-managed tenant workloads.
* Added stable per-user project names in the form `vx-<user>-<project>` and protected project state under `/usr/local/vesta/data/users/<user>/docker-projects/<project>/`.
* Added canonical Compose validation through `docker compose config --format json` and deny-first policy evaluation.
* Added constrained project lifecycle commands for create, validate, deploy, list, inspect, start, stop, restart, recreate, update, rollback, backup, restore, adopt, migrate, and remove.
* Added Vortex helper modules under `func/vx/compose/` for storage, canonicalization, policy, quotas, ownership, images, registries, secrets, volumes, networking, routes, monitoring, audit, backup/restore, transactions, migration, and web jobs.
* Added package quota fields and user counters for Docker projects, services, CPU, memory, PIDs, storage, ports, secrets, and named volumes.
* Added owner-scoped image pull/load evidence, private registry config, checksum-verified local archives, and immutable image identity recording.
* Added root-owned mode-0600 managed secrets, read-only secret mounts, encrypted-only secret backup support through `age`, and redaction tests across CLI, JSON, audit, web, logs, and backups.
* Added managed bind and named-volume support with path confinement, symlink escape rejection, verified backup archives, validation-only restore, and transactional restore rollback.
* Added project-owned networks, constrained TCP/UDP/range/localhost/public port handling, expiring administrator profiles, Vesta-owned HTTP route metadata, `vx-proxy` rendering, nginx test/reload, and Host-header route probes.
* Added normalized health, bounded logs, metrics, alerts, acknowledgement, notification fan-out, structured audit events, and Compose-aware RRD sampling.
* Added dry-run-first adoption of existing Compose projects and migration of legacy `docker.conf` direct-container records through Compose-backed compatibility adapters.
* Replaced the Docker panel workflow with owner-scoped Compose project management while preserving simple Add Container generation and adding administrator-only advanced Compose preview/deploy/update flows.
* Hardened staging and browser regression coverage, including full Compose shell suites, Playwright workflows, PHP helper tests, docs consistency checks, staging hash verification, and cleanup checks.
* Production promotion was not performed; the orchestrator remains staged and documented for explicit operator promotion only.
