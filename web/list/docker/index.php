...
$docker_quota = ($docker_owner !== '') ? vx_compose_quota_state($docker_owner, $docker_user_panel) : array(
    'limit' => null,
    'used' => 0,
    'reached' => false,
);
...
if ($docker_available) {
    $data = vx_compose_list_projects_for_actor(
        $docker_actor_is_admin ? 'admin' : $user,
        $docker_owner
    );
