    ...
    <div class="docker-shell docker-shell--list">
      ...
      <div class="docker-overview-grid">
        ...
        <article class="docker-overview-card" data-harbor-admin-status>
          ...
          <strong class="docker-overview-card__value"><?=htmlspecialchars((isset($harbor_admin_status['MODE']) ? $harbor_admin_status['MODE'] : 'disabled').' / '.(isset($harbor_admin_status['HEALTH']) ? $harbor_admin_status['HEALTH'] : 'unavailable'), ENT_QUOTES)?></strong>
          <p class="docker-overview-card__meta"><?=htmlspecialchars(__('Certificate').': '.(isset($harbor_admin_status['CERTIFICATE_STATE']) ? $harbor_admin_status['CERTIFICATE_STATE'] : 'unavailable').' · '.__('Storage').': '.(isset($harbor_admin_status['STORAGE_USED_BYTES']) ? $harbor_admin_status['STORAGE_USED_BYTES'] : 0).'/'.(isset($harbor_admin_status['STORAGE_TOTAL_BYTES']) ? $harbor_admin_status['STORAGE_TOTAL_BYTES'] : 0).' B · '.__('Provider backup').': '.__('disabled for this release').' · '.__('Pending/failed').': '.(isset($harbor_admin_status['PENDING_OPERATIONS']) ? $harbor_admin_status['PENDING_OPERATIONS'] : 0).'/'.(isset($harbor_admin_status['FAILED_OPERATIONS']) ? $harbor_admin_status['FAILED_OPERATIONS'] : 0), ENT_QUOTES)?></p>
        </article>
        <?php } elseif (!$docker_actor_is_admin && !empty($harbor_tenant_status)) { ?>
        <article class="docker-overview-card" data-harbor-tenant-status>
          <span class="docker-overview-card__label"><?=__('Managed registry')?></span>
          <strong class="docker-overview-card__value"><?=htmlspecialchars((isset($harbor_tenant_status['REGISTRY']) ? $harbor_tenant_status['REGISTRY'] : '').'/'.(isset($harbor_tenant_status['NAMESPACE']) ? $harbor_tenant_status['NAMESPACE'] : ''), ENT_QUOTES)?></strong>
          <p class="docker-overview-card__meta"><?=htmlspecialchars(__('Quota/usage').': '.(isset($harbor_tenant_status['USED_MB']) ? $harbor_tenant_status['USED_MB'] : 0).'/'.(isset($harbor_tenant_status['QUOTA_MB']) ? $harbor_tenant_status['QUOTA_MB'] : 0).' MB · '.__('Runtime').': '.(isset($harbor_tenant_status['STATE']) ? $harbor_tenant_status['STATE'] : 'unavailable').' · '.(isset($harbor_tenant_status['FRESHNESS']) ? $harbor_tenant_status['FRESHNESS'] : 'unavailable'), ENT_QUOTES)?></p>
