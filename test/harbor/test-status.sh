...
robot_query='/api/v2.0/robots?q=Level%3Dproject%2CProjectID%3D27&page=1&page_size=100'
vx_harbor_local_api_guard /run/vesta-harbor/proxy.sock GET "$robot_query" \
  || fail 'exact encoded project robot query rejected'
vx_harbor_local_api_guard /run/vesta-harbor/proxy.sock GET \
  '/api/v2.0/robots?q=Level%3Dproject%2CProjectID%3D27&page=10&page_size=100' \
  || fail 'last bounded project robot page rejected'
for value in '/tmp/proxy.sock GET /api/v2.0/health' '/run/vesta-harbor/proxy.sock POST /api/v2.0/health' '/run/vesta-harbor/proxy.sock GET /api/v2.0/configurations' '/run/vesta-harbor/proxy.sock GET /api/v2.0/projects' '/run/vesta-harbor/proxy.sock GET /api/v2.0/robots' '/run/vesta-harbor/proxy.sock GET /api/v2.0/robots?q=Level=project,ProjectID=27&page=1&page_size=100' '/run/vesta-harbor/proxy.sock GET /api/v2.0/robots?q=Level%3Dproject%2CProjectID%3D27&page=11&page_size=100' '/run/vesta-harbor/proxy.sock GET /api/v2.0/robots?q=Level%3Dproject%2CProjectID%3D27&page=1&page_size=1000'; do
