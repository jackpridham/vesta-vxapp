...
for kind in runtime publisher; do write_authority "$root/rotations/alice-$kind.json" "$(jq -cn --arg op "$operation" --arg kind "$kind" --arg now "$now" '{SCHEMA:2,OPERATION_ID:$op,OWNER:"alice",KIND:$kind,PROJECT_ID:1,ROBOT_BASENAME:($kind+"-"+$op),DESCRIPTION:("vesta-managed:vesta-harbor:alice:"+$kind+":"+$op),PHASE:"pending-revoke",NEW_ROBOT_ID:30,NEW_USERNAME:("alice-"+$kind),OLD_ROBOT_ID:10,UPDATED_AT:$now}')"; done
