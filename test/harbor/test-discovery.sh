...
write_json "$(vx_harbor_root)/observations/alice.json" "$(jq -n --arg now "$now" '{GENERATION:"g1",OBSERVED_AT:$now,USED_MB:7}')"
write_json "$(vx_harbor_root)/observations/provider.json" "$(jq -n --arg now "$now" '{HEALTH:"healthy",OBSERVED_AT:$now}')"
...
write_json "$(vx_harbor_root)/observations/alice.json" "$(jq -n --arg stale "$stale" '{GENERATION:"g2",OBSERVED_AT:$stale,USED_MB:8}')"; info="$(vx_harbor_registry_info_json alice shop)"; jq -e '.FRESHNESS=="stale" and .HEALTH=="degraded" and .USED_MB==8' <<<"$info" >/dev/null
write_json "$(vx_harbor_root)/observations/provider.json" "$(jq -n --arg stale "$stale" '{HEALTH:"healthy",OBSERVED_AT:$stale}')"; info="$(vx_harbor_registry_info_json alice shop)"; jq -e '.HEALTH=="unavailable" and .FRESHNESS=="stale"' <<<"$info" >/dev/null
write_json "$(vx_harbor_root)/observations/provider.json" "$(jq -n --arg now "$now" '{HEALTH:"degraded",OBSERVED_AT:$now}')"; info="$(vx_harbor_registry_info_json alice shop)"; jq -e '.HEALTH=="degraded"' <<<"$info" >/dev/null
