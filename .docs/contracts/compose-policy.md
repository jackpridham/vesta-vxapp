...
| `restricted-compatibility` | Admin per project | Bridge only | Managed project roots | `CHOWN`, `DAC_OVERRIDE`, `KILL`, `SETGID`, `SETUID` only |
...
The `standard`, `admin-approved`, and `restricted-compatibility` profiles reject host
networking. `standard` and `admin-approved` reject every `cap_add`;
`restricted-compatibility` accepts only `CHOWN`, `DAC_OVERRIDE`, `KILL`, `SETGID`, and
`SETUID` after an expiring root assignment. These are the exact capabilities
available for a separately reviewed compatibility workload that must prepare
bounded runtime directories, change its unprivileged service identity, and
stop supervised child processes. All projects also
