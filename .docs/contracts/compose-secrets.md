...
into a reported failure. Successful activation of a changed declared secret set
...
An identical declared generation keeps its existing inode and preserves a
service-scoped lifecycle command. Any generation change broadens that command
to project-wide convergence so no other secret-consuming service remains bound
to the prior inode.
...
For the current `compatibility-app` exact-image rehearsal, `LOG_LEVEL=info` is a
