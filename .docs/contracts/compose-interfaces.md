...
## Tenant shell-access surface

The exact tenant shell surface is brokered and owner-scoped. The broker derives
the Unix caller from `SUDO_UID` and passwd, forces `ACTOR=OWNER` and
`PROFILE=standard`, and accepts no tenant-selected identity or profile:
