...
## Section Inventory
1. Task 13: Validate And Close Out Against <staging-fqdn>
...
- Requires scratch Docker test state plus a live Playwright run covering anonymous, non-admin, and admin Docker panel flows against `https://<staging-host>:8083`.
...
- Final backend evidence shows `dockere2e/app` route metadata persisted in both `web.conf` and `docker.conf`, health/status JSON is populated, stats JSON includes populated `CPU_PCT`, `MEM_MB`, `RX_MBPS`, `TX_MBPS`, and `LATEST`, and `<test-domain>` returns the container body through nginx after the route-sync fix.
- Closeout artifacts now exist in `.docs/validation/staging-docker-e2e-closeout.md` and `<operations-repo>/Servers/<staging-jump>/<staging-fqdn>/README.md`.
