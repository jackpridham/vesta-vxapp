...
- Requires validation evidence for the shell and Playwright harness; when local Docker/Vesta runtime execution is unavailable, the fallback is shell syntax checks plus Playwright `--list`, with full runtime execution deferred to the staging closeout host.
...
- The README and Task 12 closeout now document the local-runtime fallback validation path and the deferment of full Docker-backed browser execution to Task 13 on staging.
