...
- **Authorized host:** `operator@<development-host>` (`<development-fqdn>`)
- **Authorized domain:** `admin/<application-domain>`
...
explicitly authorized deployment and testing against `<application-domain>`.
...
- **Authenticated panel:** The first Playwright attempt used the existing
  ...
  `https://<development-host>:8083`; those credentials were not valid on this
...
`<application-domain>`. The first pre-deployment `nginx -t`, run at the SSH
