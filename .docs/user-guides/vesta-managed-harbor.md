...
| Publisher identity | Distinct owner/project push-and-pull identity used only by the maintainer's builder or CI. | Harbor generates the password once. Vesta verifies it, encrypts it directly to the tenant's native age recipient, returns only armored ciphertext, and never stores publisher plaintext. It cannot administer Harbor or mutate Vesta. |
...
The tenant receives the non-secret publisher username and repository metadata
from `registry-info`, plus one age-encrypted handoff from
`registry-publisher-rotate`. The broker snapshots one bounded native X25519
age recipient from stdin; Harbor creates a new publisher password, Vesta
validates it, encrypts it in memory, switches authority, deletes the prior
publisher robot, and returns only ASCII-armored ciphertext. Vesta cannot show
the password again. A lost handoff is recovered by rerunning rotation, which
creates a replacement and revokes the inaccessible generation.
...
publisher_identity="$publisher_handoff_dir/identity.agekey"
publisher_ciphertext="$publisher_handoff_dir/publisher-secret.age"
age-keygen -o "$publisher_identity" >/dev/null
publisher_recipient="$(age-keygen -y "$publisher_identity")"
[[ "$publisher_recipient" =~ ^age1[ac-hj-np-z02-9]{58}$ ]]

printf %s "$publisher_recipient" \
  | ssh -- "$SSH_TARGET" v-docker registry-publisher-rotate \
  >"$publisher_ciphertext"
chmod 0600 "$publisher_identity" "$publisher_ciphertext"
grep -Fxq -- '-----BEGIN AGE ENCRYPTED FILE-----' "$publisher_ciphertext"
grep -Fxq -- '-----END AGE ENCRYPTED FILE-----' "$publisher_ciphertext"
...
docker login "$REGISTRY" \
  ...
  --password-stdin \
  < <(age -d -i "$publisher_identity" "$publisher_ciphertext")
jq -e --arg registry "$REGISTRY" '
  ((.auths[$registry].auth? // "") == "")
' "$docker_config_file" >/dev/null
find "$publisher_handoff_dir" -depth -delete
trap - EXIT
```

Harbor supplies the one-time password; `PUBLISHER_USERNAME` is non-secret
output from Vesta. The tenant supplies only a public age recipient and keeps
the matching identity locally. Configure Docker to use the workstation or CI platform's protected
...
base64 credential behind. Do not copy the password into a command, environment
variable, CI log, repository file, or Compose file. Keep the protected local
age files only until login completes.

If the identity, ciphertext, or Docker credential is lost, repeat
`registry-publisher-rotate` with a new local age identity. Vesta cannot recover
or display the previous password; the replacement rotation deletes the
inaccessible publisher robot after the new generation is durable.
...
- **Lost publisher credential:** Vesta cannot reveal it. Generate a new local
  age identity and rerun `registry-publisher-rotate`; Harbor generates a new
  password and the prior publisher robot is deleted after replacement
  validation.
- **Explicit disable:** `registry-publisher-disable` revokes pushes. A later
  publishing session requires a new encrypted handoff through
...
local age identity and use `registry-publisher-rotate`, then read the current
`PUBLISHER_USERNAME` again. Never reuse a password from logs, ask Vesta to
reveal one, or use the bootstrap administrator.
...
### Earlier Harbor v2.15.0 robot-secret blocker

The preserved development evidence predates this lifecycle. Source review
confirmed that Harbor generates the creation password once and that routine
robot update/refresh is unavailable to the least-privilege integration
