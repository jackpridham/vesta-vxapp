...
[[ -n "$source_package" && -n "$output_dir" && -n "$original_package" ]] \
    || fail 'usage: prepare.sh SOURCE_PACKAGE OUTPUT_DIR ORIGINAL_PACKAGE'
...
BACKUP:
sudo install -m 0600 /usr/local/vesta/data/users/slave/user.conf '$output_dir/rollback-user.conf'
sudo sha256sum /usr/local/vesta/data/users/slave/user.conf '$output_dir/rollback-user.conf'
...
ROLLBACK:
sudo /usr/local/vesta/bin/v-change-user-package slave '$original_package'
sudo /usr/local/vesta/bin/v-update-user-counters slave
sudo /usr/local/vesta/bin/v-list-docker-compose-quota slave json
sha256sum '$output_dir/rollback.pkg'
sudo cmp /usr/local/vesta/data/users/slave/user.conf '$output_dir/rollback-user.conf'
EOF
